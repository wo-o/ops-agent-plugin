...
## 접근 명령 완결성 (사용자에게 접속 명령을 반환하기 전 자기점검)

접근 부여 요청(EC2 SSH·RDS 접근 등)의 최종 산출물은 요청자가 실제로 접속하는 것이다.
`ssh`·SSH 터널·`psql` 같은 접속 명령을 응답에 실을 때는, 요청자가 그대로 복사해
실행할 수 있는 완결 명령이어야 한다 — 명령을 "생성"만 하고 완결성을 점검하지 않으면
계정·옵션·엔드포인트가 어긋난 명령이 그대로 나가 접속이 실패한다(실측: `sslmode`
누락으로 RDS가 거부). 반환 전에 다음을 점검하고, 하나라도 통과하지 못하면 그 명령을
`접속 가능`으로 제시하지 말고 미확보 항목을 분리해 보고한다.
- **실재 참조만.** 명령이 참조하는 계정·키·리소스가 실제로 발급·존재하는 것인지
  확인한다. `<user>`·`<password>` 같은 placeholder나 아직 발급되지 않은 계정을
  명령에 남기지 않는다. 값이 마스킹돼 미확보면(비밀번호 no_log 등) 그 명령을 사용
  가능으로 제시하지 않고 부분 완료로 보고한다.
- **필수 연결 옵션 완비.** 접속에 필요한 옵션을 누락하거나 잘못 설정하지 않는다.
  RDS `psql`은 반드시 `sslmode=require`로 쓴다 — RDS는 SSL을 강제하므로
  `sslmode=disable`(또는 sslmode 생략)은 `FATAL: no pg_hba.conf entry ... no
  encryption`으로 거부돼, 계정이 정상이어도 요청자가 접속하지 못한다. host/port는
  실제 터널 로컬 엔드포인트(예: `host=127.0.0.1 port=15432`)를 쓴다.
- **미검증과 완료 분리.** SG 규칙 반영·터널 TCP 성공은 요청자 PC에서의 실제 접속
  성공과 다르다. 반영이 확인된 단계만 완료로 쓰고, 실접속은 요청자 확인 권장으로 남긴다.
...
- `<env>-db-access` (RDS 접근): IP/CIDR은 필수다. 사용자가 단일 IPv4만 제공하면 의미가 명확하므로 다시 묻지 말고 `/32` CIDR로 정규화한다(예: `x.x.x.x` → `x.x.x.x/32`). `/24`~`/32`가 아닌 더 넓은 대역으로 임의 확장하지 않는다. **prod는 만료 시각(expires_at)이 필수**다.
  사용자가 기간을 지정하지 않았으면 되묻지 말고 현재 UTC 기준 **30분 후**를 기본값으로
  계산해 적용하며, 응답에서 30분 자동 만료·회수를 명시한다. 사용자가 기간이나 절대 시각을
  ...
  - 상대 만료 시각을 셸로 계산할 때 `date -d`는 명령 전체의 기준 시각을 바꾼다. 한 번의
    `date -u -d '+30 minutes'` 호출에서 `now`와 `expires`를 함께 출력하면 둘 다 만료 시각이 되는
    함정이 있으므로, 현재와 만료를 별도 호출로 읽는다:
    `printf 'now='; date -u '+%Y-%m-%dT%H:%M:%SZ'; printf 'expires='; date -u -d '+30 minutes' '+%Y-%m-%dT%H:%M:%SZ'`.
  ...
  **prod에서는 네트워크 grant만으로 접속이 안 된다** — 상시 `readonly` 계정이 없고
  ...
  사용자는 버튼을 누르거나 짧은 자연어 한 줄(예: `새 계정, temp_ro_0803, 삭제`)로 답할 수
  있고, 어느 쪽이든 완결로 처리한다 — 답변 예시를 제시할 때 `①a ②a ③a` 같은 기호 축약을
  쓰지 말고 각 선택지의 실제 라벨(예: `없음/새로 발급`·`temp_ro_0803`·`삭제`)을 그대로 보여,
  사용자가 무엇을 고르는지 라벨만 보고 알 수 있게 한다. 직접 입력 값이 오면 그 값을 쓴다.
