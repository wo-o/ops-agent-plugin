...
- prod: `expires_at` 필수. 상시 계정이 없으므로 사용자가 계정을 언급하지 않았으면 기존에
  ...
  IP·기존 계정 여부·사용자명·만료 후 role 실제 삭제(DROP) 예약 여부는 한 번에 묻되,
  자유 서술이 아니라 항목별 선택지 형식으로 묻는다 — 항목마다 구체적 옵션 + `(권장)` 표시
  + "직접 입력" 옵션, 사용자명은 짧고 읽을 수 있는 용도·날짜 기반 후보 1개(예:
  `temp_ro_0801`, Slack user ID 등 불투명 식별자 파생 금지)를 권장 옵션으로 제시
  (형식 정본은 SKILL 본문의 prod 확인 예시).
  ...
  기간을 지정하지 않으면 현재 UTC 기준 30분 후를 기본 만료로 계산하고, 기간을 다시 묻지
  않는다. 사용자가 30분 미만 만료를 직접 지정하면 머지·apply 소요로 실사용 시간이 거의
  없음을 알리고 같은 선택지 형식으로 1회 확인한다(권장 30분 — SKILL 본문 참조).
  한 번에 요청한 값들을 사용자가 하나씩 답하면 이미 받은 값은 유지하고
  아직 없는 값만 같은 선택지 형식으로 짧게 다시 묻는다. 사용자명은 소문자·숫자·`_`
  제약을 함께 안내한다.
...
5. 마지막 현재 UTC를 새로 읽어 공통 `valid_until`까지 남은 시간을 계산한다. 최종 안내에는
   단순히 "30분 사용 가능"이라고 쓰지 말고 절대 만료 시각과 전달 시점 기준 잔여 분을 쓴다.
...
- 마지막 UTC를 기준으로 남은 분을 도구로 계산하고, 최종 응답에는 “30분 사용 가능” 대신
...
기본 30분 grant는 최초 요청 시각부터 흐르므로 PR·apply·Ansible에 걸린 시간이 빠진다. 최종
응답에서 단순히 "30분 사용 가능"이라고 쓰지 말고 공통 절대 만료 시각과 전달 시점 기준의
