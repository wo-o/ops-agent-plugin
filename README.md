...
강조에 `_밑줄_`(italic)과 `**두 별표**`를 쓰지 않는다 — 둘 다 마크다운 문법이라
Slack에서 깨진다. 특히 `_..._`는 닫는 `_` 뒤에 공백·구두점이 와야 italic으로
렌더되는데, 한국어는 강조 뒤에 조사·어미가 공백 없이 붙어(예: `_1시간 뒤 만료_되며`)
닫는 `_`가 인정되지 않아 밑줄 문자가 그대로 노출된다. 강조가 필요하면 `*별표 하나*`
(Slack 볼드)만 쓰고, 애매하면 평문으로 둔다. 백틱은 코드·명령·식별자·리소스 이름을
감쌀 때만 쓰고 한국어 강조 용도로는 쓰지 않는다.
