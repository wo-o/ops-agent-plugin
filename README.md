...
밑줄 강조 `_..._`(italic)는 어떤 경우에도 쓰지 않는다 — 섹션 라벨·제목·구분을 포함해
전부. 강조·섹션 라벨은 오직 `*별표 하나*`(Slack 볼드) 하나만 쓴다. 구체적으로:
섹션 제목은 `_dev_`·`_prod_`·`_현재 상태_`가 아니라 `*dev*`·`*prod*`·`*현재 상태*`로,
값 강조도 `_1시간_`이 아니라 `*1시간*` 또는 평문으로 쓴다. `**두 별표**`(마크다운 볼드)도
금지 — Slack 볼드는 별표 하나다. 이유: `_..._`는 닫는 `_` 뒤에 공백·구두점이 와야
렌더되는데, 한국어는 강조 뒤에 조사·어미가 공백 없이 붙어(예: `_1시간 뒤 만료_되며`)
밑줄이 그대로 노출된다. 영어 단어라 렌더가 되는 경우에도 `_`는 쓰지 않는다. 백틱은
코드·명령·식별자·리소스 이름을 감쌀 때만 쓰고 강조 용도로는 쓰지 않는다.
